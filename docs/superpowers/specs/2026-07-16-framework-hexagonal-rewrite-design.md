...
5. **A frozen checklist is a verbatim record, and freezing is additive** [A1 §5.2]. The extractor records annotations as source text, but `diff_route_sets` **normalizes PEP 585 alias spelling when it compares** — so the gate is already immune to a typing sweep, and re-freezing after one only destroys the record's provenance. Therefore: never re-freeze for a formatting or typing change; freeze **additively** (`harness/freeze.py`), which keeps every surviving entry byte-verbatim, appends new routes, and **refuses** to apply a changed schema or a removed route without an explicit `--accept-drift` and a recorded reason. Regenerating a checklist to make a diff pass is the failure mode the gate exists to prevent, so widening the baseline must be a deliberate act.
...
**Amendment-1 delta:** it gained Reflex panels and its own palette-sweep test. Both are P7-UI subjects, not P5 subjects, and its dependent-surface inventory must list them. D9's corollary bites hardest here, because P5 is the phase that **deletes a duplicated estop cascade whose second copy lives in a visualizer** — a deletion that touches UI code during a non-UI phase. Scope is unchanged (§4.2.5 stands). **Three corrections, measured 2026-08-05 while planning P5** (`2026-08-05-P5-deployment-B-decomposition.md` §"Corrections"): (1) the panel count stated here as "7" is wrong and [A1 §10]'s "5 config-selected" is also off — it is **4 local config-selected panels + 1 helper**, plus a 5th config-selected name that resolves **cross-deployment** to hte's potentiostat panel, which exists in both stacks; (2) **the dual-stack estop drill condition is not met** — its station config declares no `reflex:` server (only a dev config does), so the P5 drill is Bokeh-only and its runbook needs no bundle step; (3) **its Reflex panels carry no estop buttons at all** — the Bokeh station panel has three, the Reflex port is plot-only, so a Reflex-only station would present no emergency-stop control with nothing flagging it. That last one is a **P7-UI blocker**, and it bounds what P5 may claim: after P5 only the Bokeh stack has buttons to re-point at `EstopPolicy`.
