...
**Date:** 2026-07-16
**Status:** Draft for review (master spec; each phase P0–P6 gets its own implementation plan — see §13)
...
| No legacy public-surface audit | §8: per-deployment endpoint-parity checklists generated mechanically from the audited surface maps (237 static hte endpoints + config-driven ones + the BaseAPI system surface) |
...
**Framework adapters:** artifact writers + syncer (wrapping today's `base_meta_writer`/`active_data_file`/`active_data_stream`/`active_finalizer`/`sync_driver` behaviors), dispatcher (ZMQ+HTTP), logging, config loader, NTP clock, state persistence, Bokeh vis subscribers (`vis_subscriber` stack), operator backends, sim DB server (§6.3), fakes (opt-in).
...
- **hte** (`deploy-hte-servers.md`): 23 action-server modules, **237 statically-defined endpoints** (full per-endpoint tables with params), plus config-driven `analyze_<name>` endpoints, 242 experiment functions / 13 modules, 86 sequence functions / 14 modules, 13 config-selected `*_vis` modules, operator scripts (incl. the external `data_request_client` dependency), specification parsers, meta/post processors.
...
3. **Dependent-surface inventory** (AVOID #8): per deployment, a grep-based audit of (a) experiment/sequence library imports, (b) `active.*`/`base.*` member usage, (c) private-deployment config references to shared modules, (d) `bokeh_port`-style invisible port claims — attached to the phase plan before its wave starts. Experiments/sequences libraries are **in the wave plan from day one** (their omission forced the old Wave 3.5 emergency).
...
3. Cross-cutting validation (unique keys, unique host:port, exactly one of `fast`/`bokeh`, single `params.positions` owner) stays in the launcher/preflight validator, extended with the hexagon checks (§8.3, port wiring).
...
3. Wire-format tests decode with the **real consumer's decoder** (e.g. the visualizer's WsSubscriber decode path), never a copy of the producer's encoder (the dd31c36f trap).
...
Pytest for the hexagon tree; ≥90% domain+ports coverage gate (KEEP — cheap given purity), but coverage is explicitly **not** an acceptance criterion for milestones; the parity harness + concurrency suite + endpoint checklist are. Legacy remains under its existing scripts (no pytest harness there; unchanged).
