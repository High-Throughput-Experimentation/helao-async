...
> only — **no nested-repo commits in 3c** (Deployment-B/Deployment-C code is cited as evidence, never edited).
>
...
- **(a) "drop the fallback after proving it's never exercised" is dead.** The fallback IS exercised
  by live in-repo producers: Deployment-B builds `SampleModel(..., sample_type="MEA", ...)`
  (`helao/deploy/Deployment-B/experiments/AMTS_exp.py:69`, `_mea_sample()` used in `fast_samples_in`) and Deployment-C
  builds `SampleModel(..., sample_type="xafs-std-pellet")`
  (`helao/deploy/Deployment-C/scripts/xafs/converters.py:188`). Those samples serialize into -exp/-act yml and
...
Tightening would reject them at validation. Deferred until Deployment-B/Deployment-C migrate to declared subtypes or
...
`deploy/Deployment-B/notes/**`, `deploy/Deployment-D/**` (excluded as in P1–P3); Deployment-C (constructor kwargs only —
same rule as 3a §2.1).
...
| Live fallback producers: `sample_type="MEA"`, `sample_type="xafs-std-pellet"` | Deployment-B AMTS_exp.py:69; Deployment-C xafs/converters.py:188 |
...
Commented-out sites (archive 1326, pal 1040) stay commented. Out of scope: `deploy/Deployment-B/notes/**`,
`deploy/Deployment-D/**`, Deployment-C constructor kwargs, plain local lists named `*status*`.
...
commits** — Deployment-B/Deployment-C/Deployment-A untouched.
...
- [ ] `SampleModel.sample_type` → `SampleType` enum tightening: blocked by live producers
      (Deployment-B `"MEA"` AMTS_exp.py:69; Deployment-C `"xafs-std-pellet"` xafs/converters.py:188). Decide with Deployment-B/Deployment-C
...
- [ ] Dropping the `SampleModel` fallback member outright (3e-style enforcement for samples): gated on
      ...
      the fallback exists solely for Deployment-B/Deployment-C — scope a per-deployment story.
...
- [ ] Carry-over from 3a (unchanged): base.py:1441 `set_error` field-target oddity; 3e soak window;
      Deployment-B `wait_for_temperature` divergence (3d).
