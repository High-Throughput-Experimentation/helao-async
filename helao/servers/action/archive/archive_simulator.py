...
class ArchiveSim:
    ...
    def load_plate_id(self, plate_id: int, *args, **kwargs):
        if plate_id in self.list_plates():
            if plate_id == self.loaded_plate_id:
                LOGGER.debug(f"plate {plate_id} is already loaded")
            else:
                LOGGER.debug(f"loading {plate_id} for measurement")
            ...
            LOGGER.debug(f"{plate_id} not found")
            return {}
    ...
    def acquire(self, element_fracs: list, *args, **kwargs):
        if element_fracs in self.loaded_space:
            ...
            LOGGER.debug(f"acquired sample {sample_no} with composition {compstr}")
            ...
            LOGGER.debug(f"did not find sample with composition {compstr}")
            return False
...
def makeApp(confPrefix, server_key, helao_root):
    ...
    async def acquire(
        ...
        LOGGER.debug(f"/acquire endpoint retrieved sample_no: {sample_no}")
        active.action.action_params.update(
            {
        ...
        finished_action = await active.finish()
        LOGGER.debug(f"final action_params: {', '.join(finished_action.action_params.keys())}")
        return finished_action.as_dict()
