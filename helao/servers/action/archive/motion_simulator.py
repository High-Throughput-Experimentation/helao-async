...
class MotionSim:
    ...
    def solid_get_samples_xy(self, plate_id: int, sample_no: int, *args, **kwargs):
        ...
        if len(rowmatch)==0:
            LOGGER.debug(f"Could not locate sample_no: {sample_no} on plate_id: {plate_id}")
            retxy = [None, None]
        else:
            if len(rowmatch)>1:
                LOGGER.debug(f"Found multiple locations matching plate_id: {plate_id}, sample_no: {sample_no}, returning first match.")
            else:
                LOGGER.debug(f"Found x,y")
