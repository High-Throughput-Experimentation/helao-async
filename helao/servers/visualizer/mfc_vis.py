...
class C_mfc:
    ...
    def add_points(self, datapackage_list: list):
        ...
        for mvar in self.data_dict_keys:
            if mvar.endswith("pressure") or mvar.endswith("mass_flow"):
                try:
                    ...
                    LOGGER.debug(f"datasource {self.datasource.data[mvar]}")
                    LOGGER.debug(f"data_dict {self.data_dict[mvar]}")
                    data_dict[f"{mvar}_mean"] = data_dict[mvar]
        ...
        if not self.plot.renderers or self.control_mode != control_mode:
            LOGGER.debug(f"{self.control_mode} changed to {control_mode}")
            self.control_mode = control_mode
            self._add_plots()

    async def IOloop_data(self):  # non-blocking coroutine, updates data source
        LOGGER.debug(f" ... Mass flow controller visualizer subscribing to: {self.data_url}")
        while True:
