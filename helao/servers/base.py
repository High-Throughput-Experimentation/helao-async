...
class Active:
    ...
    Methods:
        ...
        set_sample_action_uuid(self, sample: SampleModel, action_uuid: UUID):
            Sets the action UUID for the given sample.

        append_sample(self, samples: List[SampleModel], IO: str, action: Optional[Action] = None):
        ...
        track_file(self, file_type: str, file_path: str, samples: List[SampleModel], action: Optional[Action] = None):
    ...
    def set_sample_action_uuid(self, sample: SampleModel, action_uuid: UUID):
        """
        Sets the action UUID for a given sample and its parts if the sample is of type 'assembly'.

        Args:
            sample (SampleModel): The sample object for which the action UUID is to be set.
    ...
    async def append_sample(
        self, samples: List[SampleModel], IO: str, action: Optional[Action] = None
    ):
        """
        Append samples to the specified action's input or output list.

        Args:
            samples (List[SampleModel]): A list of samples to be appended.
    ...
    async def track_file(
        ...
        Args:
            ...
            samples (List[SampleModel]): A list of samples associated with the file.
