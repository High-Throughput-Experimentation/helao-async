...
class BokehOperator:
    ...
    def get_sequence_lib(self):
        ...
        self.sequences = []
        LOGGER.debug(f"found sequences: {list(self.sequence_lib)}")
    ...
    def get_experiment_lib(self):
        ...
        self.experiments = []
        LOGGER.debug(f"found experiment: {list(self.experiment_lib)}")
    ...
    def get_seqspec_lib(self):
        ...
        LOGGER.debug(f"found specs: {specfiles}")
        for fp in specfiles:
    ...
    async def get_actions(self):
        ...
        # LOGGER.debug(f"current queued actions: ({len(self.orch.action_dq)})")
    ...
    async def get_active_actions(self):
        ...
        # LOGGER.debug(f"current active actions: {self.active_action_lists}")
    ...
    def callback_clicked_pmplot(self, event, sender):
        ...
        LOGGER.debug(f"DOUBLE TAP PMplot: {event.x}, {event.y}")
    ...
    def callback_start_orch(self, event):
        if self.orch.globalstatusmodel.loop_state == LoopStatus.stopped:
            ...
            LOGGER.debug("Cannot start orch when not in a stopped state.")
    ...
    def populate_sequence(self):
        ...
        LOGGER.debug(f"selected sequence from list: {selected_sequence}")
        ...
        for k, v in sequence_params.items():
            LOGGER.debug(f"added sequence param '{k}' with value {v} and type {type(v)} ")
    ...
    def populate_experimentmodel(self) -> ExperimentModel:
        ...
        LOGGER.debug(f"selected experiment from list: {selected_experiment}")
        experiment_params = {
            paraminput.title: parse_bokeh_input(paraminput.value)
            for paraminput in self.exp_param_input
        }
        for k, v in experiment_params.items():
            LOGGER.debug(f"added experiment param '{k}' with value {v} and type {type(v)} ")
        ...
        if (
            ...
            if PMnum is not None and pmdata:
                if PMnum[0] is not None:  # need to check as this can also happen
                    LOGGER.debug(f"selected sample_no: {PMnum[0]+1}")
                    if PMnum[0] > len(pmdata) or PMnum[0] < 0:
                        LOGGER.debug("invalid sample no")
                        self.vis.doc.add_next_tick_callback(
    ...
    async def IOloop(self):
        ...
        while self.IOloop_run:
            try:
                ...
                LOGGER.error(f"BokehOperator IOloop error:", exc_info=True)
