...
class _UI:
    ...
    def __init__(self, vis, root, max_points):
        ...
        header = Div(
            ...
            styles={"font-size": "180%", "color": BROWSER_HEADER_TEXT},
            width=1000,
            height=32,
        )
    ...
    def _do_scan(self):
        ...
        try:
            ...
            self.index_df = None
            self._set_status(f"scan failed: {exc}", failed=True)
    ...
    def _set_status(self, text: str, failed: bool = False) -> None:
        """Write the status line, red on failure and default otherwise.

        The colour rides the ``Div``'s own ``styles`` rather than an inline
        ``<span style=…>``, so it resolves through the palette and clears
        itself on the next normal message.
        """
        self.status.styles = {"color": BROWSER_FAILURE_TEXT} if failed else {}
    ...
    def _on_selection_changed(self):
        ...
        self._set_status(f"{len(self.selected)} dataset(s) selected")
