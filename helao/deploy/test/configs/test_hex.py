"""``test.yml`` composed entirely of hexagon-hosted servers.

Not a copy of ``test.yml`` -- it IS ``test.yml``, loaded at launch and
...
Rollback is to launch ``test`` instead of ``test_hex``. Nothing is
migrated, nothing is deleted, and the legacy composition stays exactly as it
was.

To see what this resolves to without launching::

    python -m helao.hexagon.preflight helao/deploy/test/configs/test_hex.py
"""
...
config = hexagon_variant(os.path.join(os.path.dirname(__file__), "test.yml"))
