"""Experiment library for the ADSS station with extra-clean cleaning routines.

Mirrors ``ADSS_exp`` but adds an additional manual water injection /
recirculation / drain pass to ``ADSS_sub_clean_cell`` and adds a PAL
deep-clean after pre-aliquots in CV/OCV variants. Functions take an
``Experiment`` and return the list of actions for the orchestrator.
Action targets are referenced by ``server_key`` strings (``PSTAT``,
...
def ADSS_sub_CV(
    ...
    in-situ aliquots, and dispatches ``run_CV`` to the potentiostat. The
...
def ADSS_sub_OCV(
    ...
    """Run an OCV experiment with optional PAL aliquots and bubble-removal hook.

    Same as the ADSS variant but the pre-aliquot branch also issues a PAL
...
def ADSS_sub_clean_cell(
    ...
    """Run a clean-water flush + drain, with an optional manual water-cleaning pause.

    For ``Clean_volume_ul`` over 10000 uL the cycle is split into two
    infuse/drain passes. When ``Watercleancycle`` is True a second
    manual-injection pause followed by another pump/drain cycle is appended.
    ...
    Args:
        ...
        Watercleancycle: Run a second manual water-injection + drain pass.
