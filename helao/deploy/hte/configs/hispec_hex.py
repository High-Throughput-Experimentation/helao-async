"""``hispec.yml`` composed entirely of hexagon-hosted servers.

Not a copy of ``hispec.yml`` -- it IS ``hispec.yml``, loaded at launch and
...
Rollback is to launch ``hispec`` instead of ``hispec_hex``. Nothing is
migrated, nothing is deleted, and the legacy composition stays exactly as it
was.

To see what this resolves to without launching::

    python -m helao.hexagon.preflight helao/deploy/hte/configs/hispec_hex.py
"""
...
config = hexagon_variant(os.path.join(os.path.dirname(__file__), "hispec.yml"))
