"""``ecms3.yml`` composed entirely of hexagon-hosted servers.

Not a copy of ``ecms3.yml`` -- it IS ``ecms3.yml``, loaded at launch and
...
Rollback is to launch ``ecms3`` instead of ``ecms3_hex``. Nothing is
migrated, nothing is deleted, and the legacy composition stays exactly as it
was.

To see what this resolves to without launching::

    python -m helao.hexagon.preflight helao/deploy/hte/configs/ecms3_hex.py
"""
...
config = hexagon_variant(os.path.join(os.path.dirname(__file__), "ecms3.yml"))
