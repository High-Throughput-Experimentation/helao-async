"""``ccsi2.yml`` composed entirely of hexagon-hosted servers.

Not a copy of ``ccsi2.yml`` -- it IS ``ccsi2.yml``, loaded at launch and
...
Rollback is to launch ``ccsi2`` instead of ``ccsi2_hex``. Nothing is
migrated, nothing is deleted, and the legacy composition stays exactly as it
was.

To see what this resolves to without launching::

    python -m helao.hexagon.preflight helao/deploy/hte/configs/ccsi2_hex.py
"""
