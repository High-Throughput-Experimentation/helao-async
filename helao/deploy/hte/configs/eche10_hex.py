"""``eche10.yml`` composed entirely of hexagon-hosted servers.

Not a copy of ``eche10.yml`` -- it IS ``eche10.yml``, loaded at launch and
...
Rollback is to launch ``eche10`` instead of ``eche10_hex``. Nothing is
...
config = hexagon_variant(os.path.join(os.path.dirname(__file__), "eche10.yml"))
