"""Standalone exploratory script for the Andor camera/spectrograph stack.

Runs an end-to-end smoke test of the Andor SDK3 camera and ATSpectrograph
helpers: cool, set up the spectrograph, acquire a single image, optimise
the ND filter, run a timed acquisition burst and an SEC acquisition, then
warm down.
...
def cool(cam):
    ...
    Polls every 5 s and raises if the camera reports a temperature fault.

    Args:
        cam: ``AndorSDK3`` camera object.

    Raises:
        RuntimeError: When ``cam.TemperatureStatus`` becomes ``Fault``.
    """
...
def image_and_check_dynamic_range(cam, WL=[], exposure_time=0.0098) -> tuple:
    ...
    The optimality value is ``1 + |63000 - max| / 63000``. The range bool is
    ``True`` when the maximum pixel lies in ``[55536, 65536)``.
...
def SetupSpectroscope(
    ...
    """Initialise the spectrograph with standard parameters and return WLs.
    ...
    Returns:
        Calibrated wavelength array, or ``None`` if arguments are out of range.
    """
...
def adjust_ND(cam, WL_arr) -> tuple:
    """Sweep ND filter positions 1..6 and pick the most optimal one.

    Discards positions whose maximum pixel exceeds 54000, then applies the
    filter with the best optimality value.
    ...
    Returns:
        ``(max_array, optimality_array, ND_filter_num)``.
    """
...
def setup_SEC_aquisition(cam, exp_time=0.0098, framerate=98):
    """Configure full-AOI continuous acquisition triggered by external 5 V TTL.

    Uses rolling shutter with overlap mode, 280 MHz readout and continuous
    cycle mode.
...
def test_aquisition(
    ...
    Returns a ``DataFrame`` of triggering/buffer-wait/queue times so that
    acquisition overhead can be profiled.
...
def WarmAndClose(cam, WarmupBool):
    ...
    When ``WarmupBool`` is ``True``, disables cooling and polls until the
    sensor reaches at least 20 C with a stable status before calling
    ``cam.close()``.
    ...
    Raises:
        RuntimeError: If the camera reports a temperature fault during
            warm-up.
    """
    if WarmupBool == True:
...
def process_timings(timez, measurment_time):
    ...
    Args:
        timez: DataFrame returned by :func:`test_aquisition` with triggering,
            buffer-wait and queue times.
