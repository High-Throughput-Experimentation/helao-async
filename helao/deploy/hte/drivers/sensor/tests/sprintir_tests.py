"""Mocked pytest checks for :class:`SprintIR` initialization and CO2 acquisition flows."""
...
class TestSprintIR:
    """Mocked behavioural tests for the :class:`SprintIR` driver."""

    @pytest.mark.asyncio
    # Tests that the sensor is properly initialized and in polling mode.
    @pytest.mark.asyncio
    async def test_initialize_sensor(self, mocker):
        """Verify initialization parses the firmware values and writes polling commands."""
        # Happy path test
    ...
    async def test_acquire_co2(self, mocker):
        """Verify a successful triggered acquisition finishes and exposes scaled data."""
        # Happy path test
    ...
    async def test_sensor_not_responding(self, mocker):
        """Verify the driver finishes the action with an error when the sensor is silent."""
        # Edge case test
    ...
    async def test_co2_outside_range(self, mocker):
        """Verify the driver flags an error when the sensor returns out-of-range CO2 ppm."""
        # Edge case test
    ...
    async def test_recording_duration_zero(self, mocker):
        """Verify the driver short-circuits cleanly when duration is zero."""
        # Edge case test
        # Create a mock action and action server
        mock_action = mocker.Mock()
        mock_action.action_status = []
