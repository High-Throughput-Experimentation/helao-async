"""Linux unit tests for golden_capture_galil.py's PURE logic (no server, no HTTP).
...
Exercises only:
  ...
  - the shared (gamry-authored, imported here) ``settle`` helper, applied to
    a galil-shaped RUNS_DIAG tree

The real-hardware POST to ``/MOTOR/query_positions`` (and ``verify_device_
open``'s ``/get_status`` probe) is NOT exercised here -- that only runs
at-station (see golden_capture_galil.py / galil_diff.bat docstrings).

Written as plain assert-based ``test_*`` functions with no ``import pytest``
(mirrors ``test_golden_capture.py`` -- this repo's conda ``helao`` env does
not currently have the pytest package installed), so this module is runnable
two ways:

    conda run -n helao python -m pytest helao/hexagon/tests/smoke/test_golden_capture_galil.py -q
    conda run -n helao python -m helao.hexagon.tests.smoke.test_golden_capture_galil
"""
...
def test_snapshot_copies_parity_tops_and_writes_roundtrippable_provenance():
    with tempfile.TemporaryDirectory() as td:
        ...
        (root / "RUNS_FINISHED" / "x" / "query_position-0.hlo").write_text(
            "hlo_version: x\n%%\n"
        )
        (root / "LOGS").mkdir(parents=True)
        (root / "LOGS" / "MOTOR.log").write_text("not captured")

        out = Path(td) / "golden" / "run1"
        snapshot(root=root, out_dir=out, config_prefix="galilgold")
        ...
        assert set(QPOS_HLO_COLUMNS) == set(
            manifest.masked_hlo_columns["*query_position*.hlo"]
        )
        # "ax" (axis-name list) is config-deterministic and must NOT be masked;
        # only "position" (live encoder reading) is.
        assert "ax" not in manifest.masked_hlo_columns["*query_position*.hlo"]
...
def _write_action(root: Path, status: str = "finished", with_hlo: bool = True) -> Path:
    ...
    d = root / "RUNS_DIAG" / "25.28" / "0716" / "0__0__MOTOR__query_position"
    d.mkdir(parents=True)
    (d / "250716.131421-act.yml").write_text(
        f"file_type: action\naction_status:\n  - {status}\n"
    )
    if with_hlo:
        (d / "query_position-0.hlo").write_text("hlo_version: x\n%%\n")
    return d
