...
Port notes
----------
Import rewires from ``helao/helpers/dispatcher.py``:
- ``from .premodels import Action``          -> ``from helao.helpers.premodels import Action``
  (no framework runtime Action class exists yet — helao.framework.models.action
  holds the pure data schema only; the runtime builder stays in helpers for now)
...
Retained legacy imports (no framework equivalent / locked transitional dep):
- ``from helao.core.rpc import RPCClient, RPCSyncClient, RPCError, derive_rpc_port``
  (pure ZMQ with no server/model coupling; porting out of scope per SP7 spec)
"""
