...
These are the pure data schemas only. The runtime ``Action`` builder class
(formerly ``helao.helpers.premodels.Action``) is deliberately not referenced
here so the models layer stays free of any data->runtime circular import.
"""
...
class ShortActionModel(BaseModel, HelaoDict):
    ...
    action_server: MachineModel = Field(default_factory=MachineModel)
...
class ActionModel(ShortActionModel):
    ...
    aux_file_paths: List[Path] = Field(default_factory=list)
    from_global_act_params: dict = Field(default_factory=dict)
    to_global_params: Union[list, dict] = Field(default_factory=list)
