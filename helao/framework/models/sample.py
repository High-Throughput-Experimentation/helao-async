...
class SampleModel(BaseModel, HelaoDict):
    ...
    def create_initial_exp_dict(self) -> dict:
        ...
        The caller supplies `machine_name`; no hostname is resolved here so the
        model stays free of host/network side effects.
        """
        ...
        return {
            ...
            "machine_name": (
                self.machine_name.lower() if self.machine_name is not None else None
            ),
...
class SolidSample(SampleModel):
    ...
    def validate_global_label(self):
        """Force `global_label` to ``{machine}__solid__{plate}_{sample}`` after validation."""
        self.global_label = (
            f"{self.machine_name}__solid__{self.plate_id}_{self.sample_no}"
        )
        return self
...
def object_to_sample(data):
    ...
    Raises:
        Exception: Propagated when validation cannot match a sample type.
    """
    if isinstance(data, BaseModel):
        data = data.model_dump()
    return TypeAdapter(SampleUnion).validate_python(data)
