"""Executor four-phase contract for the action lifecycle (pure domain).

The :class:`Executor` defines the four-phase contract the action-session state
machine drives: ``_pre_exec`` (setup), ``_exec`` (one-shot work), ``_poll``
(repeated work returning status), and ``_post_exec`` (cleanup), plus a
``_manual_stop`` hook for abort. Driver and action authors either subclass this
or use the ``set_*`` setters to attach custom phase callables at runtime.

This is a near-verbatim port of ``helao.helpers.executor.Executor``, repointed
to the framework's :class:`ErrorCodes` / :class:`HloStatus`. It performs **no
I/O** — phase methods are pure async hooks; the action session injects the clock
and event-sink ports around it.

Purity: imports only from ``helao.framework.models`` / ``helao.framework.support``
and stdlib.
"""
...
class Executor:
    ...
    Attributes:
        active: The action wrapper (the :class:`ActionSession`/``Active``
            equivalent) this executor runs against.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            active: Action wrapper this executor will drive.
            poll_rate: Seconds between successive ``_poll`` invocations.
