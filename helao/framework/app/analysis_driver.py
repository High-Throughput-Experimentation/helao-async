...
Provides a single :class:`AnalysisSyncer` (standalone, no HelaoSyncer base) that
...
Framework port (Wave C): ``AnalysisSyncer`` no longer subclasses ``HelaoSyncer``;
``to_s3`` / ``dict2json`` are inlined so the class has no ``helao.core`` dependency.
"""
...
from helao.framework.support.time_utils import set_time
...
from helao.framework.adapters.loaders import pgs3
from helao.framework.adapters.loaders.localfs import LocalLoader

LOGGER = logging.make_logger(__file__) if logging.LOGGER is None else logging.LOGGER


def _dict2json(input_dict: dict) -> io.BytesIO:
    """Serialize a dict to a UTF-8 JSON byte stream rewound to position 0.

    Inlined from :func:`helao.core.drivers.data.sync_driver.dict2json`.

    Args:
        input_dict: Dictionary to serialize.

    Returns:
        A ``BytesIO`` containing the JSON bytes, ready for S3 upload.
    """
    bio = io.BytesIO()
...
class AnalysisSyncer:
    ...
    async def to_s3(
        ...
        Inlined from :class:`helao.core.drivers.data.sync_driver.SyncDriver.to_s3`
        so that :class:`AnalysisSyncer` no longer requires ``HelaoSyncer`` as a base.

        Args:
            msg: Dict to serialize to JSON, or file path to upload as-is.
            target: Destination key inside the bucket.
            retries: Number of retries (each waits 30 s) before giving up.
