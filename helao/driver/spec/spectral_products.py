...
class SM303:
    ...
    def setup_sm303(self):
        self.model = self.dll.spGetModel()
        self.dll.spTestAllChannels()
        self.dll.spSetupAllChannels()
        self.dll.spInitAllChannels(self.model)
        self.dll.spSetTEC(1)
        ...
        self.base.print_message(f"Calibrated wavelength range: {min(self.pxwl)}, {max(self.pxwl)} over {self.n_pixels} detector pixels.")
        self.wl_saved = (ctypes.c_double * 1024)()
        self.dll.spGetWLTable(ctypes.byref(self.wl_saved))

    def measure_spec(self, int_time: float, n_avg: int = 1):
        # minimum int_time for SM303 is 7.0 msec
        self.int_time = ctypes.c_double(int_time)
        self.dll.spSetTrgEx(10)
        self.dll.spSetDblIntEx(self.int_time)
        self.dll.spReadDataEx(ctypes.byref(self.data))
        data = [self.data[i] for i in range(1056)]
        data = data[10:1034]
        return data
        
    def shutdown(self):
        self.dll.spCloseAllChannels()
        self.dll.spSetTEC(0)
