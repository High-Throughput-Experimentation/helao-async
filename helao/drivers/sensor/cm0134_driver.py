    ...
    async def poll_sensor_loop(self, frequency: int = 2):
        ...
        while True:
            try:
                ...
                LOGGER.debug(f"NoResponseError: Driver polling rate is too fast. {err}")
                continue
            except serial.SerialException as err:
                LOGGER.debug(f"Device {self.config_dict['device']} is in use. {err}")
                continue
            if o2_level:
...
class O2MonExec(Executor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        LOGGER.debug("O2MonExec initialized.")
