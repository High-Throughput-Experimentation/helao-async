...
class AxisCam:
    ...
    def acquire_image(self):
        ...
        with requests.Session() as session:
            LOGGER.debug(f"making get request to {self.config_dict['axis_ip']}")
            resp = session.get(f"http://{self.config_dict['axis_ip']}/jpg/1/image.jpg")
            img = resp.content
            LOGGER.debug(f"acquired image {len(img)} at: {time.time()}")
        return img
...
class AxisCamExec(Executor):
    def __init__(self, *args, **kwargs):
        ...
        LOGGER.debug("AxisCamExec initialized.")
    ...
    async def _pre_exec(self):
        "Set flow rate."
        LOGGER.debug("AxisCamExec running setup methods.")
    ...
    async def write_image(self, imgbytes, epoch):
        ...
        LOGGER.debug(f"Writing image to: {os.path.join(self.output_dir, filename)}")
