...
class SIMDOS:
    ...
    def send(self, cmd: str):
        ...
        if not resp:
            LOGGER.debug("command did not return a valid response")
            print(full_resp)
            return None
        if len(resp) > 1:
            LOGGER.debug("command returned multiple responses, using first")
        return resp[0]
        ...
        while True:
            if self.polling:
                ...
                for group, func in [
                    ...
                    resp_dict = func()
                    # LOGGER.debug(f"received status: {resp_dict}")
                ...
                if faults:
                    LOGGER.debug(f"fault detected, stopping simdos executors")
    ...
    def set_mode(self, mode: PumpMode):
        ...
        if self.get_mode() == mode:
            ...
            LOGGER.debug(f"could not set pump mode to {mode.name}")
        return success
    ...
    def get_run_param(self, param: PumpParam):
        ...
        if resp is not None:
            ...
            LOGGER.debug(f"could not validate {param.name}")
            return -1
    ...
    def set_run_param(self, param: PumpParam, val: int):
        ...
        if val < lo_lim or val > hi_lim:
            LOGGER.debug(f"{param.name} setpoint is out of range [{lo_lim}, {hi_lim}]")
        else:
            ...
            if check is not None and int(check) == val:
                success = True
                LOGGER.debug(f"successfully set {param.name} to {val}")
            else:
                LOGGER.debug(f"could not validate {param.name} setpoint")
        return success
...
class RunExec(Executor):
    ...
    async def _pre_exec(self):
        ...
        if not setrate_resp:
            LOGGER.debug(f"could not set pump {param.name} to {val}")
            error = ErrorCodes.cmd_error
        return {"error": error}
    ...
    async def _exec(self):
        ...
        if not start_resp:
            LOGGER.debug("could not start pump")
            error = ErrorCodes.cmd_error
    ...
    async def _manual_stop(self):
        ...
        if not stop_resp:
            LOGGER.debug("could not stop pump")
            error = ErrorCodes.cmd_error
        return {"error": error}
