...
if logging.LOGGER is None:
    LOGGER = logging.make_logger(logger_name="dbpack_driver_standalone")
else:
...
class DBPack:
        ...
        while True:
            ...
            if os.path.exists(yml_target):
                try:
                    ...
                    LOGGER.error(f"Error during dbpack.finish_yml() {yml_target}.", exc_info=True)
            # else:
                ...
                if dateonly < today:
                    ...
                    if len(seq_dirs) == 0:
                        try:
                            ...
                            LOGGER.error(f"Directory {datedir} is empty, but could not removed.", exc_info=True)
                    ...
                    if len(glob(os.path.join(weekdir, "*"))) == 0:
                        try:
                            ...
                            LOGGER.error(f"Directory {weekdir} is empty, but could not removed.", exc_info=True)
    ...
    def update_log(self, yml_path: str, flag_dict: dict):
        ...
        if all(self.log_dict[yml_path].values()):
            # LOGGER.debug(f"{yml_path} is complete, removing.")
            self.rm(yml_path)
    ...
    async def finish_pending(self):
        if len(self.log_dict) > 0:
            ...
            for yml_path in yml_paths:
                LOGGER.debug(f"Finishing {yml_path}.")
    ...
    async def add_yml_task(self, yml_path: str, timeout: int = 300):
        ...
        LOGGER.debug(f"Added {yml_path} to tasks.")
    ...
    async def finish_yml(self, yml_path: Union[str, HelaoPath]):
        ...
        for target in synced_sequences:
            ...
            LOGGER.debug(f"Full sequence has synced, creating zip: {zip_target.__str__()}")
    ...
    def shutdown(self):
        # LOGGER.debug("Checking for queued DB tasks.")
        # while not self.task_queue.empty():
        #     sleep(0.2)
        # LOGGER.debug("All DB tasks complete. Shutting down.")
        pass
...
class YmlOps:
    ...
    async def to_api(self, progress_key: Union[str, int], retry_num: int = 2):
        ...
        if pdict["pending"] or not pdict["s3"]:
            LOGGER.debug("Cannot push to API with S3 upload pending.")
        ...
        LOGGER.debug(f"attempting API push for {self.yml.target.__str__()} :: {progress_key} :: {p_uuid}")
        try_create = True
        last_response = {}
                ...
                if not pdict["api"]:
                    ...
                    async with req_method(req_url, json=req_model) as resp:
                        if resp.status == 200:
                            ...
                            try_create = False
                        LOGGER.debug(f"[{i+1}/{retry_num}] {api_str} {p_uuid} returned status: {resp.status}")
                        last_response = await resp.json()
                        LOGGER.debug(f"[{i+1}/{retry_num}] {api_str} {p_uuid} response: {last_response}")
                    # await asyncio.sleep(1)

        if pdict["api"]:
            # LOGGER.debug(f"{api_str} {p_uuid} success")
            if isinstance(progress_key, str):
                self.dbp.update_log(self.yml.target.__str__(), {"api": True})
            return ErrorCodes.none
        else:
            LOGGER.debug(f"Did not post {p_uuid} after {retry_num} tries.")
            ...
            async with aiohttp.ClientSession() as session:
                for i in range(retry_num):
                    async with session.post(fail_url, json=fail_model) as resp:
                        if resp.status == 200:
                            LOGGER.debug(f"successful API push for failed {self.yml.target.__str__()} :: {progress_key} :: {p_uuid}")
                        else:
                            LOGGER.debug(f"failed API push for failed {self.yml.target.__str__()} :: {progress_key} :: {p_uuid}")
    ...
    async def _to_s3(self, msg: Union[dict, str], target: str, retry_num: int):
        ...
        for i in range(retry_num):
            try:
                ...
                # LOGGER.debug(f"Successfully uploaded {target}")
                return True
            except botocore.exceptions.ClientError as e:
                _ = "".join(traceback.format_exception(type(e), e, e.__traceback__))
                self.dbp.base.print_message(e)
                LOGGER.debug(f"Retry S3 upload [{i}/{retry_num}]: {self.dbp.bucket}, {target}")
                await asyncio.sleep(1)
        LOGGER.debug(f"Did not upload {target} after {retry_num} tries.")
        return False
    ...
    async def to_s3(self, progress_key: Union[str, int], retry_num: int = 2):
        ...
        for fpath in hlo_data:
            ...
            # LOGGER.debug(f"attempting S3 push for {file_s3_key}")
        ...
        if not pdict["s3"]:
            meta_s3_key = f"{meta_type}/{p_uuid}.json"
            # LOGGER.debug(f"attempting S3 push for {meta_s3_key}")
    ...
    def to_finished(self):
        ...
        if self.yml.status == "ACTIVE":
            for file_path in self.yml.data_files:
                ...
                # LOGGER.debug(f"moving {file_path.__str__()} to FINISHED")
                move_success = False
            ...
            # LOGGER.debug(f"cleaning up {self.yml.target.__str__()}")
            clean_success = self.yml.target.cleanup()
            if clean_success != "success":
                LOGGER.debug("Could not clean directory after moving.")
    ...
    def to_synced(self):
        ...
        if self.yml.status == "FINISHED":
            for file_path in self.yml.data_files:
                ...
                # LOGGER.debug(f"moving {file_path.__str__()} to SYNCED")
                move_success = False
                while not move_success:
                    try:
            ...
            # LOGGER.debug(f"moving {self.yml.target.__str__()} to SYNCED")
