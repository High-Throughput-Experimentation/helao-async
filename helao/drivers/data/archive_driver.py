...
class Archive:
    def __init__(self, action_serv: Base):
        ...
        LOGGER.debug(f"Config trays_dict has {len(self.startup_positions.trays_dict)} keys")
        # compare default config to backup

        try:
            self.positions = self.load_config()
            LOGGER.debug(f"Archive json has {len(self.positions.trays_dict)} keys")
        except IOError:
            LOGGER.error(f"'{self.archivejson}' does not exist, writing empty global dict.",, exc_info=True)
            self.write_config()
        # except Exception:
        ...
        if len(self.positions.trays_dict) == len(self.startup_positions.trays_dict):
            for i, tray in self.positions.trays_dict.items():
                if i not in self.startup_positions.trays_dict:
                    LOGGER.debug(f"tray key {i} from archive.json does not match config")
                    failed = True
                    break
                else:
                        ...
                        for slot in tray:
                            if slot not in self.startup_positions.trays_dict[i]:
                                ...
                                if not isinstance(
                                    ...
                                    LOGGER.error("not the same slot type", , exc_info=True)
                                    failed = True
                                    break

                    else:
                        LOGGER.error("tray has not the same length", , exc_info=True)
                        failed = True
                        break
        else:
                        ...
                        if val is not None:
                            for slot, slot_item in val.items():
                                ...
                                if slot == "slot1":
                                    ...
                                    LOGGER.error(f"unknown slot item '{slot}'", , exc_info=True)
                                    continue
                                ...
                                if slot_no is not None:
                                    ...
                                    if slot_item is not None:
                                        ...
                                        if slot_item == "VT54":
                                            ...
                                            LOGGER.error(f"slot type {slot_item} not supported",, exc_info=True)
                                            positions.trays_dict[tmpi][slot_no] = None
                                    else:
                                        positions.trays_dict[tmpi][slot_no] = None
        LOGGER.debug(f"trays: {positions.trays_dict}")
        LOGGER.debug(f"customs: {positions.customs_dict}")
        ...
        if self.archivejson is not None:
            with open(self.archivejson, "r") as f:
                try:
                    ...
                    LOGGER.error(f"error loading {self.archivejson}:", exc_info=True)
                    return Positions()
        #         data = pickle.load(f)
    ...
    async def update_samples_from_db_helper(self, sample: SampleUnion):
        ...
        if sample.sample_type is not None:
            if sample.global_label is not None:
                ...
                LOGGER.error(f"Bug found: reference sample was saved in archive.json file: {sample}",, exc_info=True)
    ...
    async def tray_export_icpms(
        ...
        if slot in trayobj.keys():
            ...
            LOGGER.debug(f"Found {len(samplelist)} vials on tray {tray}, slot {slot}. Exporting csv.")
            ...
            LOGGER.debug(f"Slot {slot} not found in positions dict. Cannot export.")
    ...
    async def tray_new_position(self, req_vol: float = 2.0, *args, **kwargs):
        ...
        async with lock:
            ...
            LOGGER.debug(f"new vial nr. {new_vial} in slot {new_slot} in tray {new_tray}")
    ...
    async def custom_unload(
        ...
        **kwargs,
        ...
        if keep_samples:
            LOGGER.info(f"trying to keep samples:  {[s.get_global_label() for s in keep_samples]}")
            tmp_sample = None
            tmp_loaded = False
            if len(keep_samples) == 1:
                ...
                if error != ErrorCodes.none:
                    LOGGER.error("no action provided to generate  assembly in order to keep samples")
                else:
    ...
    async def new_ref_samples(
        ...
        LOGGER.debug(f"source_global_label: '{source}'")
        LOGGER.debug(f"source_chemical: {source_chemical}")
        LOGGER.debug(f"source_partial_molarity: {source_partial_molarity}")
        LOGGER.debug(f"source_supplier: {source_supplier}")
        LOGGER.debug(f"source_lotnumber: {source_lotnumber}")
        ...
        if len(samples_in) == 1:
            if sample_out_type == SampleType.liquid:
                ...
                LOGGER.error(f"samples_out type {sample_out_type} is not supported yet.",, exc_info=True)
                error = ErrorCodes.not_available
    ...
    async def custom_add_liquid(
        ...
        if source_liquid_in is None:
            ...
            if not samples_in:
                LOGGER.error(f"source_liquid_in '{source_liquid_in}' is not in db",, exc_info=True)
                error = ErrorCodes.no_sample
                return error, [], []
        ...
        if custom in self.positions.customs_dict:
            ...
            LOGGER.debug(f"custom sample in valid position: {custom_sample}")
        else:
            error = ErrorCodes.not_available
            return error, [], []
        ...
        LOGGER.debug(f"sample in custom position '{custom}' is {custom_sample.exp_dict()}")
        ...
        if custom_sample == NoneSample():
            ...
            if not replaced:
                LOGGER.error("could not replace sample with assembly when adding liquid",, exc_info=True)
                error = ErrorCodes.critical
        ...
        # too, e.g. not allowed if custom is a reservoir type position
            ...
            if not samples_out2:
                ...
                LOGGER.error("could not convert reference assembly to real assembly",, exc_info=True)
        ...
        # (5-2b) liquid + assembly case
            ...
            LOGGER.debug("recovering parts from assembly")
            if loaded_liquid:
            ...
            LOGGER.debug("new assembly was successfully created")
            ref_samples_out2[0].sample_position = custom

            # a reference assembly was successfully created
            # convert it now to a real sample
            LOGGER.debug("adding new assembly to sqlite db")
            samples_out2 = await self.unified_db.new_samples(samples=ref_samples_out2)
        ...
        # custom holds a sample and we need to create an assembly
            ...
            LOGGER.error(f"Cannot add sample to position {custom}", , exc_info=True)
            return ErrorCodes.not_allowed, [], []
    ...
    async def custom_add_gas(
        ...
        if source_gas_in is None:
            ...
            if not samples_in:
                LOGGER.error(f"source_gas_in '{source_gas_in}' is not in db",, exc_info=True)
                error = ErrorCodes.no_sample
                return error, [], []
        ...
        if custom_sample == NoneSample():
            ...
            if not replaced:
                LOGGER.error("could not replace sample with assembly when adding gas",, exc_info=True)
                error = ErrorCodes.critical
    ...
    async def selective_destroy_samples(
        ...
        for sample in samples:
            # first update it from the db (get the most recent info)
            sample = await self.update_samples_from_db_helper(sample=sample)
            LOGGER.debug(f"destroy: got sample {sample.get_global_label()}")
            if sample.sample_type == SampleType.liquid:
                ...
                LOGGER.error(f"validation , exc_info=True)}' "
