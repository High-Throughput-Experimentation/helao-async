...
class HelaoAnalysisSyncer(HelaoSyncer):
    ...
    async def enqueue_calc(
        ...
        LOGGER.debug(f"Added {str(calc_tup[0])} to syncer queue with priority {rank}.")
    ...
    async def syncer(self):
        ...
        """
        LOGGER.debug("Starting syncer queue processor task.")
        while True:
            if len(self.running_tasks) < self.max_tasks:
                rank, calc_tup = await self.task_queue.get()
                LOGGER.debug(f"creating ana task for {calc_tup[0]}.")
    ...
    async def sync_ana(
        ...
        # LOGGER.debug(f"performing analysis {analysis_name}")
        # LOGGER.debug(f"using params {analysis_params}")
        if analysis_params is None:
            analysis_params = {}
        eua = ana_func(process_uuid, process_df, analysis_params)
        # LOGGER.debug("calculating analysis output")
        calc_result = eua.calc_output()
        if calc_result:
            # LOGGER.debug("exporting analysis output")
            ...
            if not self.config_dict.get("local_only", False):
                LOGGER.debug("uploading analysis model to S3 bucket")
                try:
                ...
                LOGGER.debug("Analysis server config set to local_only, skipping S3/API push.")

            outputs = model_dict.get("outputs", [])
            output_successes = []
            # LOGGER.debug("uploading analysis outputs to S3 bucket")
            ...
            if s3_model_success and s3_output_success and api_success:
                LOGGER.debug(f"Successfully synced {eua.analysis_uuid}")
                return True
    ...
    async def to_api(self, req_model: dict, retries: int = 3):
        ...
        LOGGER.debug(f"attempting API push for analysis: {meta_uuid}")
        try_create = True
        api_success = False
        last_status = 0
        last_response = {}
                ...
                async with aiohttp.ClientSession() as session:
                    for _ in range(retries):
                        async with session.post(fail_url, json=fail_model) as resp:
                            if resp.status == 200:
                                LOGGER.debug(f"successful debug API push for analysis: {meta_uuid}")
                                break
                            LOGGER.debug(f"failed debug API push for analysis: {meta_uuid}")
    ...
    async def batch_calc_dryuvis(
        ...
        while df.shape[0] == 0 and retry_counter < 3:
            LOGGER.debug("query returned 0 rows, checking again in 5 seconds.")
            time.sleep(5)
            df = pgs3.LOADER.get_recent(
                query=DRYUVIS_QUERY, min_date=min_date, plate_id=plate_id
            )
