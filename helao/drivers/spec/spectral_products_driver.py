...
class SM303:
            ...
            while self.IOloop_run:
                ...
                if self.IO_do_meas:
                    ...
                    if not self.base.actionservermodel.estop:
                        LOGGER.debug("Spec got measurement request")
                        try:
                        ...
                        if self.base.actionservermodel.estop:
                            ...
                            LOGGER.debug("setting Spec to idle")
                            # await self.stat.set_idle()
                        LOGGER.debug("Spec measurement is done")
                    else:
                ...
                if self.active is not None:
                    LOGGER.debug("Spec finishes active action")
    ...
    def setup_sm303(self):
        try:
            ...
            LOGGER.debug(f"Loaded wavelength range from EEPROM: {min(self.pxwl)}, {max(self.pxwl)} over {self.n_pixels} detector pixels.")
            self.ready = True
        except Exception as e:
            tb = "".join(traceback.format_exception(type(e), e, e.__traceback__))
            LOGGER.error(f"fatal error initializing SM303:", exc_info=True)

    def set_trigger_mode(self, mode: SpecTrigType = SpecTrigType.off):
        resp = self.spec.spSetTrgEx(mode, self.dev_num)
        time.sleep(0.1)
        if resp == 1:
            LOGGER.debug(f"Successfully set trigger mode to {str(mode)}")
            self.trigmode = mode
            return True
    ...
    def set_extedge_mode(self, mode: TriggerType = TriggerType.risingedge):
        ...
        time.sleep(0.1)
        if resp == 1:
            LOGGER.debug(f"Successfully set ext. trigger edge mode to {str(mode)}")
            self.edgemode = mode
            return True
    ...
    def set_integration_time(self, int_time: float = 7.0):
        ...
        time.sleep(0.1)
        if resp == 1:
            LOGGER.debug(f"Successfully set integration time to {int_time:.1f} msec")
    ...
    async def acquire_spec_extrig(self, A: Action):
        ...
        if trigset and edgeset and inttset:
            ...
            if not samples_in and not self.allow_no_sample:
                LOGGER.error("Spec server got no valid sample, cannot start measurement!")
                A.samples_in = []
                ...
                LOGGER.debug(f"spec_time: {self.spec_time}")
            ...
            LOGGER.error(f"Could not trigger_mode ('SpecTrigType.external'), edge_mode ({params['edge_mode']}), int_time ({params['int_time']}), or trigger_duration ({params['duration']}).")
    ...
    async def continuous_read(self):
        ...
        while self.IO_do_meas and (self.spec_time - self.start_time) < (
            ...
            try:
                ...
                self.data = []
            # if first_print:
            # LOGGER.debug(f"spReadDataAdvEx was called")
            if self.data:
                self.data = [self._data[i] for i in range(1056)][10:1034]
                # enqueue data
                ...
                # if first_print:
                #     LOGGER.debug("writing initial data")
    ...
    def shutdown(self):
        LOGGER.debug("shutting down SM303")
        # self.unset_external_trigger()
