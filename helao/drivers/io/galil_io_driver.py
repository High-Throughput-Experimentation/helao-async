...
class Galil:
    def __init__(self, action_serv: Base):
        ...
        try:
            ...
            LOGGER.error(f"severe Galil error ... please power cycle Galil "", exc_info=True)
            self.galil_enabled = False
    ...
    def shutdown(self):
        ...
        LOGGER.debug("shutting down galil io")
        self.galil_enabled = False
        try:
            self.g.GClose()
        except Exception as e:
...
class AiMonExec(Executor):
    def __init__(self, *args, **kwargs):
        ...
        LOGGER.debug("AiMonExec initialized.")
        self.start_time = time.time()
        self.duration = self.active.action.action_params.get("duration", -1)
        LOGGER.debug("AiMonExec init complete.")
