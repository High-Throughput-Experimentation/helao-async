...
:class:`~helao.helpers.premodels.Experiment`. Experiment-library functions used
to declare an ``experiment: Experiment`` first parameter that the orchestrator
passed positionally and that nested calls forwarded by hand. The decorator now
captures that experiment — whether passed positionally (by the orchestrator),
by keyword, or inherited from an enclosing experiment via
:data:`~helao.helpers.premodels.EXPERIMENT_CTX` — and exposes it through the
context var instead, so function bodies (and :class:`ActionPlanMaker`) no longer
need the parameter. Legacy functions that still declare an ``Experiment``
...
def _declares_experiment_param(sig: inspect.Signature) -> bool:
    """Return True if ``sig``'s first parameter is the parent ``Experiment``.

    Recognizes either an ``Experiment`` (sub)class annotation or a parameter
    ...
    if isinstance(ann, type) and issubclass(ann, Experiment):
        return True
...
def experiment(version: int = 1):
    ...
    extracts the parent :class:`Experiment` (passed positionally, by keyword, or
    inherited from :data:`EXPERIMENT_CTX`) and publishes it on
    :data:`EXPERIMENT_CTX` for the duration of the call, so neither the function
    signature nor nested calls need to thread it explicitly.
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            exp = None
            if args and isinstance(args[0], Experiment):
                exp, args = args[0], args[1:]
            if isinstance(kwargs.get("experiment"), Experiment):
