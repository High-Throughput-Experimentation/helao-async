...
def config_loader(confArg, helao_root):
    ...
    if confArg.endswith(".py") and os.path.exists(confArg):
        print_message(LOGGER, server_name="launcher", f"Loading config from {confArg}", info=True)
        ...
        print_message(LOGGER, server_name="launcher", f"Config not found at {os.path.abspath(confArg)}", error=True)
