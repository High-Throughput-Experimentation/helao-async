...
def load_config(confPrefix):
    ...
    if not validateConfig(pidd, confDict):
        raise Exception(Fore.RED + f"Configuration for '{confPrefix}' is invalid.")
    else:
    ...
    allGroup = {
        k: {
            sk: sv
            for sk, sv in confDict["servers"].items()
            if sv["group"] == k and sk in active
        }
    ...
    for group in A:
        ...
        for server in G:
            ...
            if codeKey:
                codeKey = codeKey[0]
                endpoints = requests.post(f"http://{S.host}:{S.port}/endpoints").json()
                filterends = [x for x in endpoints if x["name"] not in IGNORE_ENDS]
                if filterends:
    ...
    if head == "help":
        ...
        if server in cmdDict:
            if command in cmdDict[server]:
                print(
                    Fore.YELLOW
                    + f"'{command}' calls {cmdDict[server][command]['path']}"
                )
                ...
                print(Fore.YELLOW + f"use 'list {server}' to see server actions")
            else:
                print(Fore.YELLOW + f"'{command}' not available for '{server}'")
        elif server == "":
            print(Fore.YELLOW + f"valid commands are:")
            print(Fore.RESET + "\n".join(baseCommands))
        else:
            print(
                Fore.YELLOW + f"'{server}' not listed in configuration '{confPrefix}'"
            )
    elif head == "list":
        server = body.partition(" ")[0]
        command = body.partition(" ")[-1]
        if server == "":
            ...
            print(
                Fore.RESET
                + "\n".join(
                    sorted([f"{srv} {cmd}" for srv in cmdDict for cmd in cmdDict[srv]])
                )
            )
            ...
            print(
                Fore.RESET
                + "\n".join(sorted([f"{server} {cmd}" for cmd in cmdDict[server]]))
            )
        else:
            print(
                Fore.YELLOW + f"'{server}' not listed in configuration '{confPrefix}'"
        ...
        if server1 == "":
            print(
                Fore.YELLOW + f"'hotkey requires 1 or 2 motor|I/O servers to control'"
            )
        else:
            ioServ = [
                k
            ...
            if ioServ:
                ioServ = ioServ[0]
                print(Fore.YELLOW + f"using '{ioServ}' for digital I/O control")
            else:
                ioServ = None
            if motorServ:
                motorServ = motorServ[0]
                print(Fore.YELLOW + f"using '{ioServ}' for axis control")
            else:
                motorServ = None
        ...
        if server in cmdDict:
            ...
            if command in cmdDict[server]:
                S = munchify(confDict["servers"][server])
                resp = requests.post(
                    cmdDict[server][command]["path"], params=paramDict
                ).json()
                print(Style.RESET_ALL)
                pprint(resp)
            else:
