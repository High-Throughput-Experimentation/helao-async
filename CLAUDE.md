...
- **Anything transiently present in the record directory gets shipped.** Every meta and data writer here writes atomically by staging `.<name>.<uuid1hex>.tmp` beside its target and renaming it into place (`posthoc_writer.py`, `meta_writer.py`, `base_meta_writer.py`, `micro_orch.py`). A batch conversion running while SYNC globs the same directory therefore hands the uploader a name the rename has already consumed. The `.tmp`/dotfile exclusion is what closes that, and any *new* staging convention must keep one of those two shapes or it will be uploaded.
...
`helao/deploy/hte/drivers/spec/oceandirect_driver.py` + `servers/action/oceandirect_server.py`. Do not read it as a copy of the SM303 stack next door — it shares the framework shape and almost none of the device logic, and sixteen differences are traps:
...
- **The SR series has exactly three trigger modes, and they are not the FX/HDX ones.** Per *Ocean SR Series* UM-SR-Series_1025 p.19: `0` Software (host command; software sets integration time — this is the power-on mode and what a plain `get_spectrum()` uses, **not** a free-running mode), `1` External Edge (rising edge on the 16-pin IO connector, ≥10 ns pulse; software still sets integration time), `2` External Level (rising edge, and **the pulse width *is* the integration time** — whatever software set is ignored). `SRTrigMode` is named for the family deliberately: an earlier `ODTrigMode` carried the five-value FX/HDX convention, under which the obvious triggered default was `3`, a value an SR device rejects outright. The driver validates against the enum and reads the applied mode back rather than trusting the write.
- **An under-width External Level pulse returns a spectrum of all zeros and does not raise** (p.30). A flat line is the only symptom, and it is indistinguishable from a dark measurement. The pulse must be at least the device's minimum integration time (3800 µs on an SR4, i.e. `t_TLPUL` = `t_INTEG` = 3800 µs–10 s). `OceanDirectExtrigExec` counts all-zero frames into the action's `zero_frames` for exactly this reason.
- **`scans_to_average > 1` silently selects High Speed Averaging Mode, which the External Level trigger does not support** (Acquisition Mode Summary Table, p.20). HSAM runs N integrations back-to-back and returns **one** hardware-averaged spectrum; its supported triggers are Software and External Edge only. Both HSAM and the level trigger want to own the integration time and the manual does not say which wins, so `set_processing` refuses the combination rather than letting the device arbitrate.
- **The default SR4 firmware supports no external triggering at all.** p.20 documents two options: Option 1, *"the default firmware programmed on all SR4 spectrometers"*, is best for low-signal linearity and states **"External triggering is not supported. Only mode supported: Software Trigger Mode (0)"**; Option 2 supports all three modes but must be requested at order time and gives reduced low-signal linearity. The 16-pin IO connector is fitted regardless, so **the presence of the port and cable does not tell you which firmware a unit has** — arming mode 1 and seeing whether it takes does. **Settled for the station's unit (SR404456) on 2026-09-04: mode 0 takes, mode 1 is refused, so it is Option 1 and has no external triggering at all** — even though it was ordered with the trigger port and cable. Consequences: that unit has neither a data buffer nor an external trigger, so `acquire_spec_extrig` at `trigger_mode=0` is its *only* way to capture for longer than one HTTP request, and the endpoint therefore defaults to mode 0 rather than mode 1. Hardware sync on this station needs a unit whose firmware was ordered for it. Two bad defaults preceded that: `3` (an FX/HDX value no SR device accepts) and then `1`, which made the endpoint fail out of the box on the very hardware it was written for.
- **A vendor error code cannot be looked up from the docs you have, and a rejected trigger mode is indistinguishable from a transport fault.** The error-code table is not in the Python doc bundle at all: `Spectrometer.decode_error` fetches strings from the native library at runtime, and the numbering is defined in the C SDK's `OceanDirectAPIConstants.c`, which that bundle does not ship (the only two codes documented anywhere in it are `10` "missing value" and `15` for an empty required string). Compounding it, `set_trigger_mode`'s entire documentation is *"See device manual for the supported trigger mode"* — no valid values, no `@throws`, and, unlike `get_trigger_mode`, **no device-support note whatsoever** — and the Python layer forwards `mode` to the C entry point with zero validation. So an Option 1 unit refusing mode 1 as a low-level transfer error contradicts nothing documented, and the code must supply the reading itself: `_trigger_write_hint` names the likely cause, and the failure carries `trigger_mode` read back after the raise (what the device is *actually* left in) plus `reads_in_flight`. Diagnose with the mode 0 A/B — mode 0 is the power-on mode on every SR device, so if it fails too the fault is not the trigger configuration.
- **`Spectrometer.decode_error` loses the last character of every vendor message.** It sizes the buffer with `odapi_get_error_string_length(errno)` and omits the `+ 1` for the null terminator that `OceanDirectAPI.decode_error` includes. A station log reading `[6] Error: Data transfer erro` is not truncated by the logger — that is the SDK, and the clipped tail is a useful tell that the message came from a `Spectrometer` method rather than the API object.
- **An unserialized triggered read can outlive its action, and `allow_concurrent_actions` does not cover that.** `acquire_spectrum(serialize=False)` skips the driver lock so a minutes-long wait cannot serialize `disconnect()`; the justification that no second *action* can race it holds only while the action is running. Since the SDK has no acquisition timeout, `asyncio.wait_for` frees the coroutine and leaves the worker thread inside the vendor call, so a later device write can share the handle with a pending read — which the vendor renders as exactly the same unresolvable numeric code as a refusal. `reads_in_flight` counts them (own lock, since the read holds no other) and is reported by `get_status`, `device_info`, and every `set_trigger_mode` response. It deliberately **does not** make `get_status` return `busy`: a stranded read would then pin the server at busy for the rest of its life. Nothing refuses on a pending read either, because only the read returning can clear the counter, so refusing would turn a stranded thread into a lockout needing a restart.
- **Acquisition delay (`t_ACQDLY`) applies in all three modes** and is the knob for skipping a flash lamp's rise or offsetting several instruments from one trigger: 0–335,500 µs at 1 µs resolution on an SR4. Exposed as `acquire_spec_extrig`'s `acquisition_delay_us` and as a standalone `set_acquisition_delay` action; bounds are read from the device, not hardcoded, so an SR2 or SR6 needs no table here. Fixed per-acquisition overhead is `t_BUSY1` + `t_BUSY2` = 3706 µs each on an SR4, which is what caps its 74 Hz scan rate.
- **A continuous-strobe period only activates when it is shorter than the integration time** (Tables 3/6/9, note on `t_CSPER`), and a longer one is accepted by both the SDK and the device's own bounds without complaint — it just never fires, which at a station reads as dead hardware rather than as a misconfiguration. Both directions of that trap are now reported rather than refused, since configuring the strobe before lowering the integration time is a legitimate order and the device does accept the write: `set_continuous_strobe` puts `int_time_us` and `period_fires` on the finished action (`_control_action` records the driver's `data` verbatim, so the endpoint needed no change) and warns when an *enabled* strobe is inert, and `set_integration_time_us` warns when a new integration time silences a live one. The second check is judged from `continuous_strobe_period_us`/`continuous_strobe_enabled` **cached on the driver, never read back from the device** — it sits on the acquisition path, where a read-back would cost two vendor round trips per acquisition on every device including the ones with no strobe at all. `continuous_strobe_fires()` returns `None`, not a guess, when either value is unknown; a device that will not report its period cannot be judged, and a warning nobody can act on is worse than silence.
- **There are two long-running capture paths, and which one a station can use is a hardware fact.** `acquire_spec_buffered` drains the device's hardware buffer and is the only way to capture gaplessly at the detector's own frame rate; it needs `DATA_BUFFER`/`BACK_TO_BACK`, which the **OCEANSR4 does not have**. `acquire_spec_extrig` needs no buffer: it performs one blocking `get_spectrum()` per external trigger (or one per request at `trigger_mode=0`), which is how `spec_server.py`'s SM303 path works. It replaces the buffered path's *duration*, not its *rate* — software polling costs a USB round trip per spectrum and loses frames arriving between reads. Three things make the triggered path work: **the SDK has no acquisition timeout** (its only `timeout` is on `open_device2`, and `abort_spectrum_acquisition` is documented buffer-only), so the read is bounded with `asyncio.wait_for`, which frees the coroutine but leaves the worker thread blocked until a trigger arrives or the device is closed — the same trade `SM303Exec` makes; the read runs **without the driver lock** (`serialize=False`), because a minutes-long wait would otherwise serialize `disconnect()` behind it and hang shutdown; and teardown **disarms the trigger**, both because a device left armed blocks every later read on that server and because returning to software triggering is what lets a pending read complete. A timed-out poll is the normal "still waiting" state, not an error, and does not end the run.
