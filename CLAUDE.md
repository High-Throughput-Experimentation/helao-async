...
- **The operator likewise has two UIs over one `OrchBackend`.** `orch_backend.py` is the async ABC both consume; `bokeh_operator.py` is the Bokeh document (named by 32 configs, still the production UI) and `operator/app_reflex.py` is the Reflex page on `/operator`. Parameter-form logic — library introspection, `Args:` parsing, version hints, `BUILTIN_TYPES` — lives in `operator/param_forms.py` and is shared by both; add to that layer rather than to one UI. `test_standalone_operator.py`'s 48 tests are the gate on any change to the shared layer: they must pass with `bokeh_operator.py` unedited.
...
- **Known limitation: a closed browser tab leaves its poll loop running.** `on_unmount` fires on in-app navigation but not on tab close, so a `background=True` poll loop keeps polling and logs `Attempting to send delta to disconnected client` forever. Driving the tick from a client-side interval component instead of a server-side `while True` would fix it.
